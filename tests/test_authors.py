...
def test_get_authors_no_records(client, clean_up_db):
...
def test_create_author(client, token, author, clean_up_db):
...
def test_get_authors(client, sample_data, clean_up_db):
    response = client.get('/api/v1/authors')
    response_data = response.get_json()
    assert response.status_code == 200
    assert response.headers['Content-Type'] == 'application/json'
...
def test_get_authors_with_params(client, sample_data, clean_up_db):
...
def test_get_single_author(client, sample_data, clean_up_db):
...
def test_get_single_author_not_found(client, clean_up_db):
    response = client.get('/api/v1/authors/99')
    response_data = response.get_json()
...
def test_create_author_invalid_data(client, token, data, missing_field, clean_up_db):
    response = client.post('/api/v1/authors',
                           json=data,
                           headers={
                               'Authorization': f'Bearer {token}'
...
def test_create_author_invalid_content_type(client, token, author, clean_up_db):
    response = client.post('/api/v1/authors',
                           data=author,
                           headers={
                               'Authorization': f'Bearer {token}'
...
def test_create_author_invalid_missing_token(client, author, clean_up_db):
    response = client.post('/api/v1/authors',
                           json=author)
